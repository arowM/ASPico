.PHONY: build clean dump-th ghci haddock haddock-server hlint lint release repl run test watch watch-haddock watch-tests watch-test
all: build
...
release:
	...
	scp aspico.keter aspico:
	ssh -t aspico 'sudo cp aspico.keter /var/www/keter/incoming/'

run: build
	stack exec -- aspico-api
...
# Watch for changes.
watch:
	stack build --file-watch --fast aspico

# Watch for changes while trying to build haddocks
watch-haddock:
	stack build --haddock --file-watch --fast aspico

# Watch for changes.
watch-test: watch-tests
watch-tests:
	stack test --file-watch --fast aspico
